...
(function ($, EasyWeather) {
    ...
    $.fn.EasyWeather = function (opts) {
        ...
        settings = {
            ...
            geoProviderSequenceIds: 'geopl|fgip|tlz|ipinf',	// retries sequence (if geopl fails -> fgip -> tlz)
            ...
            Settings: {
                Providers: {
					...
					},
	                {
	                    ...
	                    url: function () {
	                        return EW.Helpers.protocol() +
				            '//api.wunderground.com/api/{key}/conditions/q/{query}.json';
	                    },
	                    search_url: function () {
	                        return EW.Helpers.protocol() +
	                    ...
	                    forecasts_url: function () {
	                        return EW.Helpers.protocol() +
				            '//api.wunderground.com/api/{key}/forecast10day/q/{query}.json';
	                    },
					...
					},
					{
						id: 'ipinf',
						name: 'ipinfo.io',
						description: 'IP Address API and Data Solutions',
						key: (EasyWeather.Keys.ipinf ? EasyWeather.Keys.ipinf.key() : null),
						get_link: function () {
				            return '<a href="https://ipinfo.io/" target="_blank">ipinfo.io</a>';
						},
						url: function () {
							var urlTpl = '{proto}//ipinfo.io/?callback=?&token={token}';
							return urlTpl
								.replace('{proto}', EW.Helpers.protocol())
								.replace('{token}', encodeURIComponent(this.geoProvider('ipinf').key));
						},
						...
						convert_data: function (data) {
				            ...
				            gO.country_name = data.country;
				            gO.region_name = data.region;
				            gO.longitude = latLon[1];
				            gO.latitude = latLon[0];
				            gO.ip = data.ip;
				            gO.metrocode = null;
				            gO.region_code = null;
				            gO.country_code = null;
				            gO.areacode = null;
				            gO.zipcode = data.postal;
				            return gO;
				        }
					}]
            	},
